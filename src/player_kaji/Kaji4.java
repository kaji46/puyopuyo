...
/**
 * 変更点
 * nextに対する理想最大連鎖で評価 ざこくてやめた
 *発火の時はぷよ作ったやつ使わない
 * @author Hara
 *
 */
public class Kaji4 extends AbstractPlayer {
	public Kaji4(String playerName) {
		super(playerName);
	}
					...
					for (PuyoDirection nextDir : PuyoDirection.values()) {
						...
						for (int k2 = 0; k2 < field.getWidth(); k2++) {
							...
							for (PuyoDirection nextNextDir : PuyoDirection
								...
								if (chainNum[1]  >= 5 && ojama0 == 0 && enemyNumMax + 3 < chainNum[1] ) {
									fire[1]=true;
									realfire[1]=true;
								}
								if (chainNum[2]  >= 5 && ojama0 == 0
										&& enemyNumMax + 3 <chainNum[2] ) {
									fire[2]=true;
									realfire[2]=true;
								}

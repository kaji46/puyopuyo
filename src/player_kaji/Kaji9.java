 ...
 * 
 */
public class Kaji9 extends AbstractPlayer {
	public Kaji9(String playerName) {
		super(playerName);
	}
	...
	public Action doMyTurn() {
		...
		for (int a = 0; a < 5; a++) {
			...
			mapb.put(PuyoNumber.SECOND, PuyoType.OJAMA_PUYO);
					...
					for (PuyoDirection nextDir : PuyoDirection.values()) {
						...
						for (int k2 = 0; k2 < field.getWidth(); k2++) {
							...
							for (PuyoDirection nextNextDir : PuyoDirection
								...
								chainNum[0] = chainNum(puyoColorNum(field),
										puyoColorNum(nextField), puyo);
								...
								fire[2] = false;
								if (chainNum[0] >= 5
										&& enemyNumMax + 1 < chainNum[0]) {
									fire[0] = true;
									realfire[0] = true;
								}
								// List<Integer> ojama =
								// getMyBoard().getNumbersOfOjamaList();
								// int ojama0 = ojama.get(0);

								if (chainNum[1] >= 5 && ojama0 == 0
										&& enemyNumMax + 1 < chainNum[1]) {
									fire[1] = true;
									realfire[1] = true;
								}
								...
								int score = getScore(i, dir, nextI, nextDir,
										...
										puyoAlign, ojama, oiuchi, chainNum,
										fire);
		...
		if (emergency || realfire[0] || realfire[1] || realfire[2] || oiuchi) {
	...
	private int getScore(int x, PuyoDirection dir, int nextX,
		...
		if (ojama0 > 3
			...
			score += Math.max(chainNum[1], chainNum[2]) * 5 + getPuyoNum(field)
		...
		if (oiuchi) {
			if (Math.max(chainNum[1], chainNum[0]) > 2) {
				score += Math.max(chainNum[1], chainNum[0]) * 100;
				// System.out.println("ojama1");
			}
			ConnectionCounter cnt = new ConnectionCounter(nextField);
			int[][] countField = cnt.getConnectedPuyoNum();
		...
		for (int j = 0; j < nextField.getWidth() - 1; j++) {
			if (Math.abs(nextField.getTop(j) - nextField.getTop(j + 1)) > 3) {
				score -=(Math.abs(nextField.getTop(j) - nextField.getTop(j + 1)))*0.5;
			}
		}
	...
	public int nextChainNum(Field field, Puyo[] puyoAlign) {
		...
		for (int a = 0; a < 5; a++) {
			...
			puyo.setDirection(PuyoDirection.UP);
			for (int i = 0; i < field.getWidth(); i++) {
		...
		for (int a = 5; a < 10; a++) {
			...
			for (int i = 0; i < field.getWidth(); i++) {
				...
				Field nextField = field.getNextField(puyo, i);
				nextChainNum = Math.max(nextChainNum1,
						chainNum(pcn, puyoColorNum(nextField), puyo));
				// if(chainNum(pcn, puyoColorNum(nextField), puyo)>0){
				// System.out.println("rensa");
				// break;
		...
		nextChainNum=nextChainNum*nextChainNum+(2*nextChainNum1-1)^2;

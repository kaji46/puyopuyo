 ...
 *
 */
public class Kaji3 extends AbstractPlayer {
	public Kaji3(String playerName) {
		super(playerName);
	}
	...
	private int getScore(int x, PuyoDirection dir, int nextX,
		...
		// 3連鎖以上する場合は積極的に置く
		if (chainNum >= 5
				&& enemyNumMax + 3 < getPuyoNum(field) - getPuyoNum(nextField)) {
			score += (getPuyoNum(field) - getPuyoNum(nextField));
			score *= 3;
		}
		// List<Integer> ojama = getMyBoard().getNumbersOfOjamaList();
		...
		if (nextChainNum >= 5 && ojama0 == 0 && enemyNumMax + 3 < nextChainNum) {
			score += (getPuyoNum(field) - getPuyoNum(nextField));
			score *= 2;
		}
		if (nextNextChainNum >= 5 && ojama0 == 0
				&& enemyNumMax + 3 < nextNextChainNum) {
			score += (getPuyoNum(field) - getPuyoNum(nextField));
			score *= 2;
		}

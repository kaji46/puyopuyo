 ...
 *
 */
public class Kaji extends AbstractPlayer {
	public Kaji(String playerName) {
		super(playerName);
	}
	...
	public Action doMyTurn() {
		...
		Action action = new Action(PuyoDirection.DOWN, 2);
		int maxScore = 0;
						...
						for (int k2 = 0; k2 < field.getWidth(); k2++) {
							...
							for (PuyoDirection nextNextDir : PuyoDirection
								...
								int score = getScore(i, dir, nextI, nextDir,
										nextNextI, nextNextDir, enemyNum,puyoAlign);
								...
								puyoScore puyoscorek=new puyoScore(score,actionk,nextNextNextField);
...
//								}
		...
		for(int b=0;b<100;b++){
			int score=puyoscore.get(b).getScore()+nextChainNum(puyoscore.get(b).getField(),puyoAlign)*10;
			if(maxScore<score){
				maxScore=score;
...
//この辺ももってこれる
		...
		if (ojama0 > 0
				|| totalPuyoNum > field.getWidth() * field.getHeight() - 15) {

			// 危機的状況の時は積極的に消しに行く
			score += chainNum * 5 + getPuyoNum(field) - getPuyoNum(nextField);
			// System.out.println("ojama1");
			return score;

		} else if (ojama1 > 0
				|| totalPuyoNum > field.getWidth() * field.getHeight() - 17) {
